...
class CacheManager:
    ...
    def save(
        ...
        if split_date is not None:
            ...
            if train_df is not None:
                ...
                print(f"学習用データをキャッシュに保存: {cache_paths['train']}")

            # テスト用データを保存
            if test_df is not None:
                self._save_dataframe(test_df, cache_paths["test"])
                print(f"テスト用データをキャッシュに保存: {cache_paths['test']}")

            # 評価用データを保存
            if eval_df is not None:
                self._save_dataframe(eval_df, cache_paths["eval"])
                print(f"評価用データをキャッシュに保存: {cache_paths['eval']}")
        else:
            if converted_df is not None:
                ...
                print(f"前処理済みデータをキャッシュに保存: {cache_paths['data']}")
        ...
        if featured_df is not None:
            ...
            print(f"特徴量抽出データをキャッシュに保存: {cache_paths['featured']}")
        ...
        if converted_df is not None and split_date is not None:
            ...
            print(f"変換済みデータをキャッシュに保存: {cache_paths['converted']}")
    ...
    def load(
        ...
        if not metadata_path.exists():
            print(f"[DEBUG] キャッシュメタデータが見つかりません: {metadata_path}")
            return None
        ...
        try:
            ...
            if (
                ...
                print(f"[DEBUG] キャッシュのデータタイプまたは年度が一致しません")
                print(f"  メタデータ: data_types={metadata.get('data_types')}, year={metadata.get('year')}")
                print(f"  リクエスト: data_types={data_types}, year={year}")
                return None
            ...
            if split_date is not None:
                ...
                if cache_paths["train"].exists():
                    ...
                    print(f"学習用データをキャッシュから読み込み: {cache_paths['train']} ({len(train_df):,}件)")

                if cache_paths["test"].exists():
                    test_df = self._load_dataframe(cache_paths["test"])
                    print(f"テスト用データをキャッシュから読み込み: {cache_paths['test']} ({len(test_df):,}件)")

                if cache_paths["eval"].exists():
                    eval_df = self._load_dataframe(cache_paths["eval"])
                    print(f"評価用データをキャッシュから読み込み: {cache_paths['eval']} ({len(eval_df):,}件)")
                ...
                if cache_paths["data"].exists():
                    ...
                    print(f"前処理済みデータをキャッシュから読み込み: {cache_paths['data']} ({len(df):,}件)")
                    return df
                else:
                    return None
    ...
    def clear_cache(
        ...
        if data_types is not None and year is not None:
            ...
            for path in self._cache_dir.glob("*.json"):
                path.unlink()
            print(f"すべてのキャッシュを削除: {self._cache_dir}")
