...
def extract_all_features_unified(
    ...
    stats_df = sed_df_with_key[stats_cols].copy()
    ...
    if "血統登録番号" in result_df.columns and "血統登録番号" in stats_df.columns:
        print("馬ごとの特徴量抽出中...")
        result_df = _extract_horse_features_unified(result_df, stats_df)
    
    # 騎手ごとにgroupbyして、1回の時系列フィルタリングで全ての特徴量を計算
    if "騎手コード" in result_df.columns and "騎手コード" in stats_df.columns:
        print("騎手ごとの特徴量抽出中...")
        result_df = _extract_jockey_features_unified(result_df, stats_df)
    
    # 調教師ごとにgroupbyして、1回の時系列フィルタリングで全ての特徴量を計算
    if "調教師コード" in result_df.columns and "調教師コード" in stats_df.columns:
        print("調教師ごとの特徴量抽出中...")
        result_df = _extract_trainer_features_unified(result_df, stats_df)
    
    result_df.index = original_index
    return result_df


def _extract_horse_features_unified(target_df: pd.DataFrame, stats_df: pd.DataFrame) -> pd.DataFrame:
    """
    ...
    with tqdm(total=len(valid_groups), desc="馬特徴量抽出", leave=True, unit="頭") as pbar:
        for horse_id in valid_groups:
            ...
            result_df = _process_horse_group_unified(horse_id, group_stats, group_targets)
            if len(result_df) > 0:
...
def _process_horse_group_unified(
...
def _extract_jockey_features_unified(target_df: pd.DataFrame, stats_df: pd.DataFrame) -> pd.DataFrame:
    """
    ...
    with tqdm(total=len(valid_groups), desc="騎手特徴量抽出", leave=True, unit="人") as pbar:
        for jockey_id in valid_groups:
            ...
            result_df = _process_jockey_group_unified(jockey_id, group_stats, group_targets)
            if len(result_df) > 0:
                results.append(result_df)
            pbar.update(1)
...
def _process_jockey_group_unified(
...
def _extract_trainer_features_unified(target_df: pd.DataFrame, stats_df: pd.DataFrame) -> pd.DataFrame:
    """
    ...
    with tqdm(total=len(valid_groups), desc="調教師特徴量抽出", leave=True, unit="人") as pbar:
        for trainer_id in valid_groups:
            ...
            result_df = _process_trainer_group_unified(trainer_id, group_stats, group_targets)
            if len(result_df) > 0:
                results.append(result_df)
            pbar.update(1)
...
def _process_trainer_group_unified(
