...
describe('scrapeJRACalendarWithRaceResults', () => {
  ...
  it('正常なリクエストで成功し、カレンダーとレース結果データを取得・保存できる', async () => {
    // モックモジュールをインポート
    const { fetchJRAHtmlWithPlaywright } = await import('../src/utils/htmlFetcher')
    const { saveRacesToFirestore } = await import('../src/utils/firestoreSaver')
    ...
    // HTMLフェッチャーをモック設定
    // カレンダーHTMLを最初に返し、その後開催日分のレース結果HTMLを返す
    vi.mocked(fetchJRAHtmlWithPlaywright)
      .mockResolvedValueOnce(mockCalendarHtml) // カレンダーHTML
      .mockResolvedValueOnce(mockRaceResultHtml) // レース結果HTML（1日分）

    // Firestore保存をモック設定
    vi.mocked(saveRacesToFirestore).mockResolvedValue(10)
    ...
    expect(saveRacesToFirestore).toHaveBeenCalled()
  })
})
